...
import PicsAtlas from "../assets/images/pics.js";
import ErasData from "../assets/fonts/eras.js";
require("./wx");
...
function Config() {}
Config.RELEASE = !1, Config.PIANO = !0, Config.VERSION = "0.99", Config.WIDTH = 640, Config.HEIGHT = 712, Config.FONT_LEVEL_SELECT_ID = "Arial", Config.FONT_GAMEPLAY_LEVEL_ID = "Titan One", Config.TOUCH_BOUNDS = {
    x: 0,
    y: 100,
    width: Config.WIDTH,
...
App.Parser = Class.extend({
    ...
    parseLevelData: function (a) {
        ...
        if (d)
            for (b = 0; b < d.length; b++) this.addStaticLink(d[b].start, d[b].end, d[b].arrow);
...
App.Commands = Class.extend({
    init: function () {},
    gameStart: function () {
        Config.TEST_GAME_COMPLETE ? cmd.gameComplete() : cookies.isNewGame ? cmd.levelStart(Config.START_LEVEL) : game.state.start("TierSelect");
    },
    tierSelect: function (a) {
        data.curTier = data.tiers[a], game.state.start("LevelSelect");
    },
...
App.Cookies = Class.extend({
    ...
    levelComplete: function () {
        ...
        this.info.maxLevel = a, this.info.isNewGame = !1, this.updateOpenLevels();
        this.save();
    },
    clear: function () {
...
App.Data = Class.extend({
    init: function (a) {
        this.lang = a, this.texts = weixin.readFile('src/data/text.json', 'json'), this.curLevel = null, this.levelStartTime = 0, this.levelStartTime = 0, this.pauseTime = 0, this.pausedByUser = !1;
        this.atlasName = "pics";
        this.initTiers();
    ...
    initLevels: function () {
        ...
        weixin.downloadLevel(this.curTier.id +1);
    },
    addLevel: function (a) {
    ...
    initTiers: function () {
        this.tiers = [];
        var levelData = weixin.readFile('src/data/tier.json', 'json');
        levelData.forEach(element => {
...
App.GameLogic = Class.extend({
    ...
    touchInsideBounds: function (a, b) {
        var c = Config.TOUCH_BOUNDS.x,
            d = Config.TOUCH_BOUNDS.y - Config.offsetY,
...
App.GameplayState.prototype = {
    create: function () {
        ...
        this.logic = new App.GameLogic(this.game, this.coreGroup, a.dotsGroup, a.linksGroup, a.dots, a.staticLinks), this.uiGroup = new Phaser.Group(this.game, this.game.world), this.uiGroup.fixedToCamera = !0, this.restartBt = new App.MyButton(this.game, "restartButton", this.restartClick, this, {
            ...
            x: 197,
            y: 56 + Config.offsetY + Config.HEIGHT + (Config.bottom),
            parentGroup: this.uiGroup
...
App.LevelSelectState.prototype = {
    ...
    homeClick: function () {
        fader.fade(this.gotoTier, this)
    },
    gotoTier: function () {
        this.game.state.start("TierSelect");
    }
};
...
App.MainMenuState.prototype = {
    create: function () {
        ...
        this.moreGames.bt.visible = this.creditsBt.bt.visible = this.deleteProgressBt.bt.visible = !1;
...
App.PreloaderState.prototype = {
    preload: function () {
        this.add.sprite(0, -Config.offsetY, "background");
        var a, b = ["fader", "blackBack", "staticLink", "userLink", "drawLine", "pauseBox", "gameCompleteBack.jpg", 'tier_complete', 'tier_current', 'tier_lock', ],
        ...
        if (Config.SOUNDS_ENABLED) {
            ...
            for (a = 0; a < c.length; a++) {
                this.load.audio(c[a], Config.cdnUrl + "assets/sounds/" + c[a] + ".mp3");
            }
        }
        this.load.atlasJSONHash("pics", "assets/images/pics.png", null, PicsAtlas);
        this.load.bitmapFont("eras", "assets/fonts/eras.png", null, ErasData);
        return;
        this.holder = this.game.add.group(), this.preloadBar = this.add.sprite(60, 269, "preloadBar", null, this.holder), this.add.sprite(60, 264, "preloadBarHolder", null, this.holder), this.progressTxt = this.game.add.text(Config.WIDTH / 2, 210, "0%", {
...
var startup = function () {
    ...
    if (Math.abs(Config.ratio - 16 / 9) > 0.01) {
        ...
        Config.iphonex = true;
        Config.bottom = Config.WIDTH * Config.ratio - Config.HEIGHT - Config.offsetY - 130;
        Config.levelY = 70;
    } else {
        Config.offsetY = 200;
